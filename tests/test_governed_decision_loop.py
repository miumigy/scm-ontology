...
class ReplenishmentProvider:
    ...
    def reason(self, reasoning_input):
        return ReasoningOutput(context_id=reasoning_input.context_id, proposal={"action":"replenish","quantity":10}, rationale="warehouse stock is below threshold", evidence_ids=reasoning_input.evidence_ids, provenance_ids=reasoning_input.provenance_ids, confidence=0.95)


def test_governed_decision_loop_is_end_to_end_and_deterministic():
    observation = GraphReasoningObservation(question_id="warehouse-stock", value={"warehouse":"WH-1","stock":5,"threshold":10}, evidence_ids=("e-stock-1",), provenance_ids=("p-erp-1",))
    ...
    decision = authorize_decision(proposal, actor_id="planner-1", authority="supply-chain-manager", authorized_at="2026-08-17T21:00:00Z")
    command = build_execution_command(decision, command_type="replenishment", command_id="cmd-371-001")
    assert command.context_id == "ctx-371"
    assert command.decision.proposal.output.evidence_ids == ("e-stock-1",)
    assert command.decision.proposal.output.provenance_ids == ("p-erp-1",)
