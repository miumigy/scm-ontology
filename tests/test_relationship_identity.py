import pytest
from scm_ontology.relationship_identity import RelationshipIdentityError, RelationshipInstance, is_relationship_instance
...
def test_relationship_has_stable_identity_and_endpoints():
    ...
    assert is_relationship_instance(relationship)

@pytest.mark.parametrize("field", ["relationship_id", "from_id", "predicate", "to_id"])
def test_rejects_empty_identity_fields(field):
    values = {"relationship_id":"R1", "from_id":"order-1", "predicate":"supplied_by", "to_id":"supplier-a"}
    values[field] = ""
