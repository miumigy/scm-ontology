            ...
            "transitions": [
                {
                    ...
                    "changes": copy.deepcopy(dict(t.changes)),
                }
...
class SimulationKernel:
    ...
    def apply_event(self, state: State, event: Event) -> tuple[State, Transition]:
        ...
        if event.event_type != SUPPLIER_DELAY_LEAD_TIME_RULE.event_type:
            raise SimulationError(f"Unsupported event type: {event.event_type}")

        next_state, changes = apply_transition_rule(
            state, event, SUPPLIER_DELAY_LEAD_TIME_RULE
        )
        transition_id = _stable_id(
            {
                "event": event.event_id,
                "from": state.state_id,
                "to": next_state.state_id,
            }
        )
...
def _event_dict(event: Event) -> dict[str, Any]:
    ...
    if event.provenance is not None:
        data["provenance"] = {
            "causedByEventId": event.provenance.caused_by_event_id,
            "ruleId": event.provenance.rule_id,
            "causalDepth": event.provenance.causal_depth,
        }
    return data
