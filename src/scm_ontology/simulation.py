...
class SimulationRun:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "simulationRunId": self.simulation_run_id, "scenarioId": self.scenario_id, "seed": self.seed,
            "initialState": self.initial_state.snapshot(),
            "events": [_event_dict(e) for e in self.events],
            "transitions": [{"id": t.transition_id, "eventId": t.event_id, "eventType": t.event_type,
                             "fromStateId": t.from_state_id, "toStateId": t.to_state_id,
                             "entityId": t.entity_id, "changes": copy.deepcopy(dict(t.changes))} for t in self.transitions],
            "finalState": self.final_state.snapshot(),
        }
    def canonical_payload(self) -> dict[str, Any]:
        return {"simulationRun": self.to_dict(),
                "events": [{"id": e.event_id, "eventType": e.event_type, "occurredAt": e.occurred_at, "entityId": e.entity_id} for e in self.events],
...
class SimulationKernel:
    ...
    def apply_event(self, state: State, event: Event) -> tuple[State, Transition]:
        ...
        if event.occurred_at < state.effective_at:
            raise SimulationError("Event time cannot precede state effective time")
        entity = copy.deepcopy(dict(state.entities[event.entity_id]))
        changes: dict[str, dict[str, Any]] = {}
        if event.event_type == "SUPPLIER_DELAY":
            if entity.get("entityType") != "Party" or entity.get("partyType") != "SUPPLIER":
                raise SimulationError("SUPPLIER_DELAY requires a supplier Party entity")
            magnitude = event.attributes.get("magnitudeDays")
            if not isinstance(magnitude, int) or magnitude < 0:
                raise SimulationError("SUPPLIER_DELAY magnitudeDays must be a non-negative integer")
            before = entity.get("leadTimeDays")
            if not isinstance(before, int) or before < 0:
                raise SimulationError("Supplier leadTimeDays must be a non-negative integer")
            after = before + magnitude
            entity["leadTimeDays"] = after
            changes["leadTimeDays"] = {"before": before, "after": after}
        else:
    ...
    def run(self, scenario: Scenario) -> SimulationRun:
        ...
        for event in ordered_events:
            state, transition = self.apply_event(state, event); transitions.append(transition)
        run_id = _stable_id({"scenario": scenario.scenario_id, "seed": scenario.seed,
                             "initialState": scenario.baseline_state.snapshot(),
...
def _event_dict(event: Event) -> dict[str, Any]:
    return {"id": event.event_id, "eventType": event.event_type, "occurredAt": event.occurred_at,
