...
class CanonicalGraphQueryBoundary:
    ...
    def relationships(
        ...
        *,
        ...
        graph = self._load(graph_id, graph_version)
        try:
            return query_canonical_relationships(
                graph, relationship_type=relationship_type, node_id=node_id
            )
        except GraphQueryError as exc:
            raise CanonicalGraphQueryError(str(exc)) from exc
...
def query_canonical_nodes(
    ...
    *,
    ...
    nodes = tuple(
        ...
        for n in sorted(
            (n for n in graph.nodes if (node_id is None or n.node_id == node_id)
    ...
    relationships = tuple(
        ...
        for r in sorted(
            (r for r in graph.relationships if r.instance.from_id in ids or r.instance.to_id in ids),
