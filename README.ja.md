...
![SCM Ontology コンセプト](./assets/scm-ontology-concept-en.png)
...
SCM Ontologyは、こうした表現の違いと、その先にあるグラフ・推論アプリケーションとの間に、**意味を統一・統制するための境界**を置きます。

中心となる原則は次の一文です。

> **基準となる事実は、統治された手続きを経て成立する。対応付け、類似性、推論、投影、取り込みが成功しただけで、基準となる事実を暗黙に変更してはいけない。**

## SCM Ontologyとは

SCM Ontologyは、特定の企業、業界、製品、ERP / WMS / TMS / APSなどの用語体系から独立して、サプライチェーンを表す**対象、関係、出来事、状態、制約、意思決定、KPI、リスク**などを定義する、機械可読な基準意味モデルです。

企業の根拠情報をこのモデルに対応付けることで、異なるシステムから得られた情報を共通の意味で扱い、同じSCM上の対象について横断的に推論できるようにします。

重要なのは、**対応付けたことと、それを基準となる事実として認めたことを分ける**ことです。
...
SCM OSは、SCM Ontologyの上に構築される**統治の仕組みを備えたSCM運用レイヤー**です。
...
**SCM Ontologyの意味モデルと、統治されたSCM OSリファレンス実装は、一次公開に必要な基盤が整っています。**

リファレンス実装では、観測、意思決定の文脈構築、ルールおよびLLMによる推論、提案の検証、承認・統治、外部副作用を伴わない実行、業務ワークフロー、監査・再現、永続グラフ基盤（リレーショナル / Neo4j）、閉ループ実行、複数情報源の参照データ対応、限定された自律制御までを、一連の統治された流れとして扱えます。

これは**本番向けのすべてのコネクタやグラフ基盤を実装済みという意味ではありません**。v0.1.0は、SCM OntologyとSCM OSの考え方を第三者が理解・実行・拡張できるようにした、リファレンス実装としての公開版です。
...
一次公開では、内部の多数の開発番号を追うのではなく、**標準実行経路を入口とします**。
...
新しい環境から、一次公開の標準実行経路と受入確認を実行できます。
...
標準実行経路では、統治されたリファレンス実装を一連の決定的な処理として実行します。外部への副作用はなく、基準となる事実を暗黙に変更しません。
...
各事実の版には、再構築に必要な**出所、情報源の識別、適用範囲、時間に関する基準、ライフサイクル履歴、統治上の判断**を保持します。
...
3. **出所を残す** — 情報源と根拠を統治された結果とともに保持します。
...
`registry/` は機械可読な形にし、`schemas/` は構造を検証し、`fixtures/` は回帰検証を
支え、`examples/` は利用者がどのように動かすかを示します。
...
- 一次公開後のアイデアに対して、新しいPhaseや`Sxxx`番号を作ることから始めないでください。まず**一次公開を妨げる要因**、**重要な改善**、**一次公開後のバックログ**のどれかに分類し、必要なら [`BACKLOG.yaml`](BACKLOG.yaml) に記録してください。
