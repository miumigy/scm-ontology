...
トップレベルのディレクトリは、それぞれ安定した役割を持ちます。意味ごとに
情報を一個所へ置くことは、このプロダクトの仕様の一部です。
...
| `src/` | Pythonによる実装（パッケージ `scm_ontology`）。 |
| `tests/` | 意味と統治の境界を守る回帰テスト。 |
| `docs/` | 人間向け文書（アーキテクチャ、仕様、一次公開、開発履歴）。 |
| `mappings/` | 対応表（例：APICS/SCOR）を基準モデルから分離して保持。 |

この構成は認知の流れを反映しています。`ontology/` は意味を定義し、
`registry/` は機械可読にし、`schemas/` は構造を検証し、`fixtures/` は回帰検証を
支え、`examples/` は利用者がどう動かすかを示します。
