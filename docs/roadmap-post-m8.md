...
the governed loop to an authorized execution command and dry run. S360 resolves a demand/supply shortage into a procurement decision through the governed loop, S361 resolves a production requirement against capacity into a scheduling decision, and S362 resolves a shipment requirement against transportation capacity into a distribution decision. S358–S362 form the first Phase R5 application set across the physical material flow: replenish -> procure -> produce -> distribute. S363 is the first Phase 5 slice: it composes those four governed applications into a bounded, multi-period, multi-decision simulation, running each step through the S348 governed loop with an S353 dry run in a deterministic, side-effect-free run. S364 is the planning/optimization slice for replenishment: it extends the single-period replenishment decision into a multi-period, cost-aware replenishment plan that is optimized deterministically and executed period-by-period through the governed loop. S365 extends the same planning/optimization pattern to the remaining R5 applications (procurement S360, production S361, distribution S362), so the Phase 5 planning/optimization integration spans the full physical material flow: replenish -> procure -> produce -> distribute. S366 is the visualization & operational workflow slice: it consumes governed decision output and turns it into an operational workflow, auditing each decision (S354) and advancing its command lifecycle (S355) to the dry-run state, then folding everything into a deterministic workflow report.
...
S363 composes the Phase R5 applications into a governed, multi-step simulation. It reuses the S348 governed loop, the S351 rule-based provider, and the S353 execution runtime for every step, introduces no new canonical semantics, and performs no external side effect. S364 extends the single-period replenishment decision into an optimized multi-period replenishment plan: it deterministically computes the lot-for-lot replenishment quantities that minimize holding cost while avoiding stockouts, records the result as a `Plan` (replenishment_plan), and runs each period through the governed loop. S365 applies the same deterministic multi-period planning/optimization pattern to procurement (shortage-matching, `procurement_plan`), production (within-capacity scheduling, `production_plan`), and distribution (within-capacity shipping, `distribution_plan`), each run period-by-period through the governed loop. S366 closes the loop after a decision: given governed decision output, it records a content-addressed audit entry (S354), starts and advances the command lifecycle through the dry-run state (S355), and produces an immutable workflow report with per-step state and a workflow-level summary.

### Phase 5 — checklist

- [x] S363 governed simulation application (multi-period, multi-decision);
- [x] S364 optimized replenishment plan application (planning/optimization integration);
- [x] S365 optimized procurement/production/distribution plan applications (remaining R5 applications);
- [x] S366 operational workflow application (visualization & operational workflow integration);
